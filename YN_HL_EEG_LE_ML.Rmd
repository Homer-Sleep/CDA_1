...
toRemove = c('SNR', 'SAE', 'PCE', "SNR_num","SNR_inver",'SNR_plot',"Neuroeconomic", "Proxy1", "WIN Test Level", 'MoCA Extra Point?', "Adj_MoCA")
...
### Test new model for multicolliniearty 
...
### First Attempt: Random forest - no boosting/bagging - predicting Mean peak LE SNR


```{r,fig.width=8, fig.height=10}
#remove unwanted variables:
# colnames(EEG.db[['train']])
Unwanted = c('Sub', "Alpha.d1", "LTheta.d1","Alpha.peak","LTheta.peak",  "Theta_24","Theta_20","Theta_16","Theta_12", "Theta_8",      
...
# Make predictions
predictions <- model1 %>% predict(EEG.db[['test']][colnames(EEG.db[['test']]) %in% wanted])
# Model performance
data.frame(
  RMSE = RMSE(predictions, EEG.db[['test']][colnames(EEG.db[['test']]) %in% wanted]$Mean.LE.SNR),
  R2 = R2(predictions,EEG.db[['test']][colnames(EEG.db[['test']]) %in% wanted]$Mean.LE.SNR)
)

car::vif(model1)
```

With ceiling.SNR, RLE.d1, and WIN_Score_16 having very high to questionable VIF, lets revisit the lasso with these items removed. 
