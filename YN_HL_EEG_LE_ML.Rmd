...
2) Within >0 70% SNRs, find the SNR that generates the largest first derivative of Alpha, Late Theta, ILE and RLE
3) Within >0 70% SNRs, find the SNR that generates peak (positive) of Alpha and Late Theta
...
```{r}
Unwanted = c( 'Sub', "Alpha.d1", "LTheta.d1","Alpha.peak","LTheta.peak",  "Theta_24","Theta_20","Theta_16","Theta_12", "Theta_8", "Theta_4","Theta_0","Late.Theta_24", "Late.Theta_20", "Late.Theta_16", "Late.Theta_12", "Late.Theta_8" , "Late.Theta_4",
             "Late.Theta_0",  "Alpha_24","Alpha_20","Alpha_16","Alpha_12","Alpha_8","Alpha_4","Alpha_0")
wanted = c('MoCA', 'L4000','Ceiling.SNR','RLE.d1','WIN_Score_16','Mean.LE.SNR' )
...
```{r}
Unwanted = c('Mean.LE.SNR', 'Sub', "Alpha.d1", "LTheta.d1","Alpha.peak","LTheta.peak",  "Theta_24","Theta_20","Theta_16","Theta_12", "Theta_8", "Theta_4","Theta_0","Late.Theta_24", "Late.Theta_20", "Late.Theta_16", "Late.Theta_12", "Late.Theta_8" , "Late.Theta_4",
             "Late.Theta_0",  "Alpha_24","Alpha_20","Alpha_16","Alpha_12","Alpha_8","Alpha_4","Alpha_0")
...
Unwanted = c(Unwanted, Multico, alias_list)
  ...
  rsq = 1-rss/tss
  print(paste('Test R Squared = ', round(rsq, digits = 3)))
  print(plot(Mean.LE.SNR.test,pred, abline(coef = c(0,1)), ylim = c(10, 24), xlim = c(10, 24) ))
                        
lasso_var = c('Education', 'L4000', 'R2000', 'R6000', 'ILE.d1', 'ILE_0', 'RLE_24', 'RLE_4', 'WIN_Score_24', 'WIN_Score_20')
```
...
if (sum(wanted %in% c("Mean.LE.SNR" , "Education","L4000","R2000","R6000","ILE.d1","ILE_0","RLE_24","RLE_4", "WIN_Score_24", "WIN_Score_20")) == 11){
  print('variables consistent with previous runs')
}else{
 stop('Variables in consistent with prevous runs, check alias_list and lasso results')
 }
...
EEG.model = randomForest(formula = Mean.LE.SNR ~ ., 
                         data = EEG.db[['train']][colnames(EEG.db[['train']]) %in% wanted], ntree = ntree.best, mtry = mtry.best, importance=TRUE)
...
pred = predict(EEG.model, EEG.db[['test']][colnames(EEG.db[['test']]) %in% wanted])
  ...
  tss = sum(( EEG.db[['test']]$Mean.LE.SNR - mean( EEG.db[['test']]$Mean.LE.SNR ))^2)
  rsq = 1-rss/tss
  paste('Test R Squared = ', round(rsq, digits = 3))
  plot(Mean.LE.SNR.test,pred, abline(coef = c(0,1)), ylim = c(10, 24), xlim = c(10, 24) )
    # Find Test RMSE
  MRSE = sqrt(mean((pred -  EEG.db[['test']]$Mean.LE.SNR)^2))
  paste('Test RMSE = ',round(MRSE, digits = 3))
...
A linear model has a $$R^2$$ = 0.338 and the random forest has typical $$R^2$$ = 0.663. This shows that the data are non-linear and that a random forest approach can better capture and predict the Mean LE SNR outcome. 
