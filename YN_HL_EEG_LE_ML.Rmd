...
path = "\\\\v09.med.va.gov\\mou\\Service\\RES\\RESRepository\\_Protocol Data Storage\\AVRes\\07-180s - Aud Cog\\Listening Effort EEG Study\\Results\\Single Trial\\"
# path = "C:\\Dave\\EEG_LE\\Results\\"
# 
# EEG_path = path

EEG_path <- "\\\\v09.med.va.gov\\mou\\Service\\RES\\RESRepository\\_Protocol Data Storage\\AVRes\\07-180s - Aud Cog\\Listening Effort EEG Study\\Results\\Single Trial\\No_ICA_ExpFreq\\multi_electrode\\"
...
# PrePhrase_filename = "Pre-PhraseTrialArray_Ep_-1.5_-0.85.csv"

Phrase_filename = "PhraseTrialArray_Ep_-0.8_0.csv"

Late_filename ="LateTrialArray_Ep_0.4_1.1.csv"

PTA_Demo_Raw=read.csv( "EEG_LE_Data_N=34_PTA_Demo.csv")

## load in Self-Report and PC data
PC_SR_Raw=read.csv( "EEG_LE_Data_N=34_RLE_ILE.csv")
...
colnames(PTA_Demo_Raw)[colnames(PTA_Demo_Raw)=="Subj."] = "Sub"
colnames(PTA_Demo_Raw)[colnames(PTA_Demo_Raw)=="MoCA.Raw.Score"] = "MoCA"
PTA_Demo_Raw$Sub=gsub("YN0", "YN_", PTA_Demo_Raw$Sub)
PTA_Demo_Raw$Gender=as.factor(PTA_Demo_Raw$Gender)
PTA_Demo_Raw$Handedness=as.factor(PTA_Demo_Raw$Handedness)

# rename to help with sep=6
colnames(PC_SR_Raw)[colnames(PC_SR_Raw)=="R.LE.LISN"] = "LISN__RLE"
colnames(PC_SR_Raw)[colnames(PC_SR_Raw)=="I.LE.LISN"] = "LISN__ILE"
# convert to long format and split on 6th char in Variable names

PC_SR=PC_SR_Raw %>% gather(var, SR, LISN__RLE:WIN_R2_0dB_ILE)%>%
  separate(var, c("Run", "Condition"), sep = 6)


# remove leading and trailing "_"  and rename subj's to replace 0 with _
# help http://www.endmemo.com/program/R/gsub.php
PC_SR$Condition=gsub("^_", "", PC_SR$Condition)
PC_SR$Run=gsub("__$","", PC_SR$Run)
PC_SR$Run=gsub("_$","", PC_SR$Run)
PC_SR$Subj.=gsub("YN0", "YN_", PC_SR$Subj.)
...
PC_SR_tib = as_tibble(PC_SR) 
# arrange by subj., works! 
PC_SR_tib=PC_SR_tib%>% arrange(Subj.)
# make wide format for self reports
PC_SR_tib=PC_SR_tib%>% spread(Score_type, SR)
# reorder columns
PC_SR_tib=PC_SR_tib[c(1,3,4,5,6,7,2)]
# rename columns
colnames(PC_SR_tib)[colnames(PC_SR_tib)=="Score"] = "WIN_Score"
colnames(PC_SR_tib)[colnames(PC_SR_tib)=="Subj."] = "Sub"
...
## Fix YN_12 ILE and RLE scores
PC_SR[PC_SR[,1] == 'YN_12',]$ILE = round(PC_SR[PC_SR[,1] == 'YN_12',]$ILE, digits = 0)
PC_SR[PC_SR[,1] == 'YN_12',]$RLE = round(PC_SR[PC_SR[,1] == 'YN_12',]$RLE, digits = 0)

##########################
## Load EEG data

EEG_LE_Late =read.csv(Late_filename)
# EEG_LE_Late = dplyr::filter(EEG_LE_Late, T2 !="NA")

EEG_LE_Word =read.csv( Word_filename)
# EEG_LE_Word = dplyr::filter(EEG_LE_Word, T2 !="NA"& Run != "Baseline")
# 
# EEG_LE_PreCon =read.csv(paste0(EEG_path, PreCon_filename))
# EEG_LE_PreCon = dplyr::filter(EEG_LE_PreCon, T2 !="NA")

# EEG_LE_PrePhrase =read.csv(paste0(EEG_path , PrePhrase_filename))
# EEG_LE_PrePhrase = dplyr::filter(EEG_LE_PrePhrase, T2 !="NA" & Run != "Baseline")

EEG_LE_Phrase =read.csv( Phrase_filename)
# EEG_LE_Phrase = dplyr::filter(EEG_LE_Phrase, T2 !="NA")

## artifact rejection, rbind() to combine and unique() to remove duplicates

# loop through each Epoch for rejection
Epoch_List_Raw = list(EEG_LE_Phrase ,EEG_LE_Word ,EEG_LE_Late) # move to list
rm(EEG_LE_Phrase ,EEG_LE_Word ,EEG_LE_Late) # rm 


Epoch_List = Epoch_List_Raw
 # lapply(Epoch_List, "[", ,8:12, FUN = mean)
for (i in seq_along(Epoch_List_Raw)){
  
    this_epoch=Epoch_List_Raw[[i]][,1:5]
    this_epoch$Theta=(rowMeans(Epoch_List_Raw[[i]][,8:12]))
    this_epoch$Alpha= (rowMeans(Epoch_List_Raw[[i]][,17:21]))
...
for (i in seq_along(Epoch_List)){
 ...
 this_Over_T=Epoch_List[[i]]
 this_Over_T=(dplyr::filter(Epoch_List[[i]], FzMaxAmp>Threshold | PzMaxAmp>Threshold | F3MaxAmp>Threshold | P3MaxAmp>Threshold | F4MaxAmp>Threshold | P4MaxAmp>Threshold))
