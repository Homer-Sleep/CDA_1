...
# create variable to go through anova data frame and select the models that are significant for Holms correction and tables. 
Anova.result = T_Result
holmsB = function(Anova.result){ # sequential Bonferroni adjustment (a.k.a. Holm-Bonferroni method) 
  model.names = row.names(Anova.result)
  for (this.model in model.names) {
    
    
  }
  
  tTable = summary(this.model)[['tTable']]
  # https://stats.stackexchange.com/questions/225937/linear-mixed-effects-model-and-multiplicity-issue-and-adjusting-for-p-values
  k = nrow(tTable)-1
  X = matrix(0, nrow(tTable), ncol = 3)
  colnames(X) = c('rank','Adj.α','Adj.p')
