...
Audiogram = dplyr::select(PTA_Demo, Sub, Group,L250:R8000) # select audiogram info
    ...
    Audio.Mean = data.frame('Sub' = Audiogram$Sub, 'Group' = Audiogram$Group, 250,500,1000, 2000, 3000, 4000, 6000, 8000)
    # mean of L and R values
    for (i in 1:8) {
      left.col = i+2
      right.col = i+10
      Audio.Mean[,i] = rowMeans(Audiogram[,c(left.col,right.col)])
    }
    
    #remove L&R
    

Audiogram = pivot_longer(dplyr::select(PTA_Demo, Sub, Group,L250:R8000),
                         cols = L250:R8000,
                         names_to = 'Freq',
                         values_to = 'Thresh',
                        names_transform = list(Freq = as.numeric, Thresh = as.integer)
                        )
                         
                         

gg_pta(Audiogram)+
  aes(x=Freq, y=Thresh, group=Group, shape=21)+
