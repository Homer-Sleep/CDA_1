...
gg_pta(Audio.Mean, ylab = "Hearing Level in Decibels (dB)", xlim = c(125, 8900))+
...
Freq_plot_IAF = NULL
 for (i in seq_along(Epoch_List)){
   # change alpha theta
       Freq_plot[[i]] = Epoch_List[[i]]%>% dplyr::select( Sub, Theta, Alpha, SNR_plot, Group)%>%pivot_longer(cols = Theta:Alpha, 
                                           names_to = "Frequency",
                                      names_transform = list(Frequency = ~readr::parse_factor(.x, levels = c("Theta", "Alpha"))),
                                           values_to = "Power")
       Freq_plot_IAF[[i]] = Epoch_List[[i]]%>% dplyr::select( Sub, Theta_IAF, Alpha_IAF, SNR_plot, Group)%>%pivot_longer(cols = Theta_IAF:Alpha_IAF, 
                                           names_to = "Frequency",
