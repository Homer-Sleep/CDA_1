...
ILE_SNR_Paradigm_Theta_Alpha.int <- lm(ILE ~ poly(SNR, degree = 3, raw = TRUE) * Paradigm * Theta * Alpha, random=~1|Sub,
                 data=filter(Epoch_List[[3]], Response != "NoRes", Group == 'ONH'), method="ML")

summ(ILE_SNR_Paradigm_Theta_Alpha.int, center = T)


# Create data used for model with polynomials
lm.data=filter(Epoch_List[[3]], Response != "NoRes", Group == 'ONH')# loop by SNR?
lm.data$SNR2 = lm.data$SNR^2
lm.data$SNR3 = lm.data$SNR^3

probe_interaction(ILE_SNR_Paradigm_Theta_Alpha.int, pred = Theta, modx = Alpha, mod2 = SNR, alpha = .05) # pred = first predictor, modx = x axis (line type), mod2 = panels
