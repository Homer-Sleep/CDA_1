...
```{r, echo=FALSE}
# jtools approach
summ(ILE_SNR_Paradigm_Theta_Alpha.int, center = T)
# Use lm cause lme output does not work:
ILE_SNR_Paradigm_Theta_Alpha.int <- lm(ILE ~ poly(SNR, degree = 3, raw = TRUE) * Paradigm * Theta * Alpha, random=~1|Sub,
                 data=filter(Epoch_List[[i]], Response != "NoRes", Group == group_i), method="ML")

# Create data used for model with polynomials
lm.data=filter(Epoch_List[[i]], Response != "NoRes", Group == group_i)
lm.data$SNR2 = lm.data$SNR^2
lm.data$SNR3 = lm.data$SNR^3

probe_interaction(ILE_SNR_Paradigm_Theta_Alpha.int, pred = Alpha, modx = Theta, mod2 = SNR, alpha = .05, data = lm.data) # pred = first predictor, modx = x axis (line type), mod2 = panels
probe_interaction(ILE_SNR_Paradigm_Theta_Alpha.int, pred = Alpha, modx = Theta, alpha = .05, data = lm.data)
```
