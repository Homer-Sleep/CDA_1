...
# copies of data are located here:
...
# trace(gg_pta, edit=TRUE)
# untrace(gg_pta)



holmsB = function(tTable, alpha.level = 0.05){ # sequential Bonferroni adjustment (a.k.a. Holm-Bonferroni method) 
  ...
  k = nrow(tTable)-1
  X = matrix(0, nrow(tTable), ncol = 4) # empty matrix for new info
  colnames(X) = c('rank','Adj.α','Adj.p', 'Significant')
  tTable = as.data.frame(cbind(tTable, X)) #make data.frame as ifelse() below and indexing didn't work for matrix
  tTable[2:nrow(tTable),6]= rank(tTable[2:nrow(tTable),5]) # rank() numbers items in ascending order
  # (1+k−i)∗pi ; k = comparisons; i = rank ascending, pi = given p-value
  tTable[,8]= (1+k - tTable[,6]) * tTable[,5] # new adjusted alpha level
  tTable[,7] = alpha.level/(1+k - tTable[,6]) # new adjusted p-values
  tTable[,9] = ifelse(tTable[,8] < alpha.level, TRUE, FALSE) # label  non-sig models FALSE
    nonSigrank = min(tTable[!tTable[,9],]$rank) # find lowest rank of non-sig models, labeled FALSE
  # if there are non-sig models, label all with higher rank than lowest non-sig rank as non-sig
  if (nonSigrank == Inf || nonSigrank < 1){
    }else{
  tTable[,9] = ifelse(tTable$rank < nonSigrank, TRUE, FALSE) 
    }
    tTable[1,9] = '-'
  return(tTable)
}
...
# holmsB_GLMM = function(T_Result, Table.Omnibus = F, Table.holms = T, alpha.level = 0.05){ 
#   # sequential Bonferroni adjustment (a.k.a. Holm-Bonferroni method) with kable output
#   # https://stats.stackexchange.com/questions/225937/linear-mixed-effects-model-and-multiplicity-issue-and-adjusting-for-p-values
# 
#   # table Omnibus results:
#   if (Table.Omnibus == T){
...
#   }
#   
#   # post-hoc adjustment within linear model:
#   Modelname = rownames(T_Result)
#   for (holms_i in 2:(nrow(T_Result))){
#   if (T_Result[holms_i,9] > alpha.level){ next} # next iteration if omnibus > alpha.level
#   tTable = (summary(get(Modelname[holms_i]))[['tTable']]) # retrieve tTable from linear model lme()
#   k = nrow(tTable)-1 # get number of variables in model
#   X = matrix(0, nrow(tTable), ncol = 4) # empty matrix for new info
#   colnames(X) = c('rank','Adj.α','Adj.p', 'Significant')
#   tTable = as.data.frame(cbind(tTable, X)) #make data.frame as ifelse() below and indexing didn't work for matrix
#   tTable[2:nrow(tTable),6]= rank(tTable[2:nrow(tTable),5]) # rank() numbers items in ascending order
...
#     tTable[1,9] = '-'
# 
#   # make table or make list output:
#   if (Table.holms == T){
#   print(kable(tTable, format = "html", escape = F, caption = paste0('<b>',Modelname[holms_i],'<b>'))%>%
#   kable_styling( full_width = F)%>%
#   column_spec(1,color="black", bold=T)%>%
#   row_spec(0:nrow(tTable),color = "black"))
#     }else{
#       #Make a list for each loop output
#       if (!exists ('holms.tables')){
#       holms.tables = list(tTable)
#       Table_i = 1
#       }else{
#       Table_i = Table_i + 1
#       holms.tables[[Table_i]] = tTable
#       }
#     }
#   }
#   if (exists('holms.tables')){
...
GLMM_Tables = function(T_Result, Table.Omnibus = T, Table.model = T, Results = F, alpha.level = 0.05){ 
   # table Omnibus results:
  if (Table.Omnibus == T){
    print(kable(T_Result, format = "html", escape = F ) %>%
  kable_styling( full_width = F)%>%
  ...
  # find highest rank with significant p value
  sig.model = max(T_Result[T_Result[,9] <= alpha.level& !is.na(T_Result[,9]),]$Model)
  if (sig.model== -Inf) {
    print('No Significant Models found')
    return()
  }
  Modelname = rownames(T_Result[sig.model,])

  tTable = (summary(get(Modelname))[['tTable']]) # retrieve tTable from linear model lme()
  X = matrix(0, nrow(tTable), ncol = 1) # empty matrix for new info
  colnames(X) = c( 'Significant')
  tTable = as.data.frame(cbind(tTable, X)) #make data.frame as ifelse() below and indexing didn't work for matrix
  tTable[,6] = ifelse(tTable[,5] < alpha.level, TRUE, FALSE) # label  non-sig models FALSE
  tTable[1,6] = '-'
...
PTA_Demo_Raw=as.data.frame(read_excel( "EEG_LE_CDA_Data.xlsx", sheet = 'Enter Data Here'))
...
EEG_LE_Phrase_Raw =read.csv(paste0(path, Phrase_filename))
EEG_LE_Phrase_Raw = dplyr::filter(EEG_LE_Phrase_Raw, T2 !="NA" )

EEG_LE_Word_Raw =read.csv(paste0(path, Word_filename))
EEG_LE_Word_Raw = dplyr::filter(EEG_LE_Word_Raw, T2 !="NA" )

EEG_LE_Late_Raw =read.csv(paste0(path, Late_filename))
EEG_LE_Late_Raw = dplyr::filter(EEG_LE_Late_Raw, T2 !="NA")

EEG_LE_BaseCon_Raw =read.csv(paste0(path, BaseCon_filename))
...
for(ii in seq_along(Epoch_List)){
  ...
  # component data taken from EEG structures in MATLAB
Components.mean = round(describe(c(5,10,13,9,5,14,1,7,5,11,10,2,7,5,7,8,6,6)), digits = 1)[,c(3,4,8,9)]
 kable(Components.mean, row.names = F, caption = 'Components Removed')%>%
...
PeakTable$RPA= names(PeakTable)[z]
  ...
  PeakTable$Participant=1:nrow(PeakTable)
    ...
    PTA_Demo$Group = factor(PTA_Demo$Group, levels = c('OHL', 'ONH'))
    names(PTA_Demo)[4] = 'Veteran'
    PTA_Demo$Veteran = gsub('NO', 'No', PTA_Demo$Veteran)
    factor.cols = c('Veteran','Gender', 'Ethnicity', 'Race', 'Handedness' )
    PTA_Demo[factor.cols] = lapply(PTA_Demo[factor.cols], factor)
...
```{r, echo=FALSE}
Audiogram = dplyr::select(PTA_Demo, Sub, Group,L250:R8000) # select audiogram info

    factor.cols = colnames(Audiogram[3:18]) # get names for conversion
    Audiogram[factor.cols] = lapply(Audiogram[factor.cols], as.numeric) #Convert to numeric
    # Audio.Mean = data.frame( 'Group' = unique(Audiogram$Group), 'X250' = NA,'X500' = NA ,
    #                         'X1000' = NA, 'X2000' = NA, 'X3000' = NA, 'X4000' = NA, 'X6000' = NA, 'X8000' = NA,
    #                         'SD250' = NA,'SD500' = NA ,'SD1000' = NA, 'SD2000' = NA, 'SD3000' = NA, 'SD4000' = NA, 'SD6000' = NA, 'SD8000' = NA)
    
 Audio.Mean = data.frame( 'Group' = rep(unique(Audiogram$Group), times= 1, each = 8), 'Freq' = rep(c(250, 500,1000,2000,3000,4000,6000,8000), 2),
                           'Threshold' = NA, 'SD' = NA)
    Freqs = c('250', '500','1000','2000','3000','4000','6000','8000')

    # mean and SD of L and R values
    for (group in (Audio.Mean$Group)) {
     for (i in 1:length(Freqs)) {
      this.threshold =  as.vector(as.matrix(Audiogram[Audiogram$Group == group,endsWith(colnames(Audiogram),Freqs[i])]))
      Audio.Mean[Audio.Mean$Group == group & Audio.Mean$Freq == Freqs[i], 3] = describe(this.threshold)$mean
      Audio.Mean[Audio.Mean$Group == group & Audio.Mean$Freq == Freqs[i],4] = describe(this.threshold)$sd
    }
   }
...
gg_pta(Audio.Mean, ylab = "Hearing Level in Decibels (dB)", xlim = c(225, 8900), ylim = c(90, -10))+
  aes(x=Freq, y=Threshold, group=Group, fill = group, shape = 21)+
  geom_line(  size=1.3)+
  geom_errorbar(aes(ymin=Threshold-SD, ymax=Threshold+SD), size=.8,width=.3,
                position = position_dodge(width = 0.15))+
  geom_point(size=4, aes(fill = Group))+
  ...
  theme(axis.title.y = element_text(color= 'black',face='bold',size=13),
...
Demo.summary = describeBy (PTA_Demo, PTA_Demo$Group)
group.demo = round(Demo.summary[[1]][c(3,8,10,30:37),c(3,4,8,9)], digits = 1)
kable(group.demo, format = "html", escape = F, caption = 'OHL Demographics' ) %>%
  kable_styling( full_width = F)%>%
  column_spec(1,color="black", bold=T)%>%
  row_spec(0:nrow(group.demo),color = "black")
# table(PTA_Demo[PTA_Demo$Group =='OHL',factor.cols[-5]]) # Only one left handed, so remove handedness with -5

group.demo = round(Demo.summary[[2]][c(3,8,10,30:37),c(3,4,8,9)], digits = 1)

kable(group.demo, format = "html", escape = F, caption = 'ONH Demographics' ) %>%
  kable_styling( full_width = F)%>%
  column_spec(1,color="black", bold=T)%>%
  row_spec(0:nrow(group.demo),color = "black")

# table(PTA_Demo[PTA_Demo$Group =='ONH',factor.cols[-5]])
...
# create paradigm factor
 PC_df$Paradigm <- NA
  PC_df$Paradigm[PC_df$Run %in% c("WIN_1", "WIN_2")] <- 'Descending'
  PC_df$Paradigm[PC_df$Run %in% c("WIN_R1", "WIN_R2")] <- 'Randomized'
  PC_df$Paradigm = factor(PC_df$Paradigm, levels = c('Descending','Randomized'))
...
```{r, echo=FALSE,results='asis'}
## debuging
# setdiff(unique(Epoch_List[[1]]$Sub), unique(PC_SR$Sub))
# setdiff(unique(PC_SR$Sub), unique(Epoch_List[[1]]$Sub))
###
...
WIN_Score_Group_SNR.main <- lme((WIN_Score) ~ Group+SNR, random=~1|Sub,
                 data=dplyr::filter(PC_df, !Run %in% c("Baseline", "WIN_P")), method="ML")

WIN_Score_Group_SNR.int<- lme((WIN_Score) ~ Group*SNR, random=~1|Sub,
                 data=dplyr::filter(PC_df, !Run %in% c("Baseline", "WIN_P")), method="ML")

WIN_Score_Group_SNR_Paradigm.main <- lme((WIN_Score) ~ SNR*Group+Paradigm, random=~1|Sub,
                 data=dplyr::filter(PC_df, !Run %in% c("Baseline", "WIN_P")), method="ML")

WIN_Score_Group_SNR_Paradigm.int <- lme((WIN_Score) ~ SNR*Group*Paradigm, random=~1|Sub,
                 data=dplyr::filter(PC_df, !Run %in% c("Baseline", "WIN_P")), method="ML")


T_Result=anova(WIN_Score_NoCon, WIN_Score_Group, WIN_Score_Group_SNR.main,WIN_Score_Group_SNR.int,
               WIN_Score_Group_SNR_Paradigm.main,WIN_Score_Group_SNR_Paradigm.int) 


GLMM_Tables(T_Result, Table.Omnibus = T)
#################################################################################################################################################### Model fixed and untested. Fix and test remaining GLMM
```
...
WIN_Score_SNR_Paradigm <- lme((WIN_Score) ~ SNR*Paradigm, random=~1|Sub,
                 data=dplyr::filter(PC_df, !Run %in% c("Baseline", "WIN_P"), Group == "ONH"), method="ML")
...
## Colors:
Red.blue =  c( 'dodgerblue2', 'red2')
white.black = c('white', 'black')
for (this.color in Red.blue) {
...
#scale_shape_discrete(solid=T) +
  scale_shape_manual(values=c(22,24,25,21, 23, 22 ))+
   theme_classic()+
geom_point(data=d, mapping=aes(x=a, y=0.5, shape=a, fill="black"), size=4)+
  scale_fill_manual(values = this.color))
...
#scale_shape_discrete(solid=T) +
  scale_shape_manual(values=c(22,24,25,21, 23, 22 ))+
   theme_classic()+
geom_point(data=d, mapping=aes(x=a, y=0.5, shape=a, fill="black", color = this.color), size=4, stroke = 1.6)+
  scale_fill_manual(values = 'white')+
  scale_color_manual(values = this.color))
...
PC_plot=ggplot()+
  ...
  geom_point(data=dplyr::filter(PC_df, Group == "ONH", Condition != "LISN", Paradigm == 'Descending'), aes(x=SNR_plot, y=PerCor),
             shape=21, fill = "black",stat="summary", fun=mean, size = 4)+
  geom_line (data=dplyr::filter(PC_df, Group == "ONH", Condition != "LISN", Paradigm == 'Descending'), aes(x=SNR_plot, y=PerCor),
              stat="summary", fun=mean, group=10, linetype='solid', size=1.3)+
  stat_summary(data=dplyr::filter(PC_df, Group == "ONH", Condition != "LISN", Paradigm == 'Descending'), aes(x=SNR_plot, y=PerCor),
                 geom = "errorbar",fun.data = mean_se, color="black",size=.7,width=.2)+

  
        
  geom_line (data=dplyr::filter(PC_df, Group == "ONH", Condition != "LISN", Paradigm == 'Randomized'), aes(x=SNR_plot, y=PerCor),
              stat="summary", fun=mean, group=10, linetype='solid', size=1.3)+
  stat_summary(data=dplyr::filter(PC_df, Group == "ONH", Condition != "LISN", Paradigm == 'Randomized'), aes(x=SNR_plot, y=PerCor),
                  geom = "errorbar",fun.data = mean_se, size=.7,width=.2)+
  geom_point(data=dplyr::filter(PC_df, Group == "ONH", Condition != "LISN",Paradigm == 'Randomized'), aes(x=SNR_plot, y=PerCor),
...
PC_plot=ggplot()+
  ...
  geom_point(data=dplyr::filter(PC_df, Group == "OHL", Condition != "LISN", Paradigm == 'Descending'), aes(x=SNR_plot, y=PerCor),
             shape=21, fill = "black",stat="summary", fun=mean, size = 4)+
  geom_line (data=dplyr::filter(PC_df, Group == "OHL", Condition != "LISN", Paradigm == 'Descending'), aes(x=SNR_plot, y=PerCor),
              stat="summary", fun=mean, group=10, linetype='solid', size=1.3)+
  stat_summary(data=dplyr::filter(PC_df, Group == "OHL", Condition != "LISN", Paradigm == 'Descending'), aes(x=SNR_plot, y=PerCor),
                  geom = "errorbar",fun.data = mean_se, size=.7,width=.2)+

  
        
  geom_line (data=dplyr::filter(PC_df, Group == "OHL", Condition != "LISN", Paradigm == 'Randomized'), aes(x=SNR_plot, y=PerCor),
              stat="summary", fun=mean, group=10, linetype='solid', size=1.3)+
  stat_summary(data=dplyr::filter(PC_df, Group == "OHL", Condition != "LISN",Paradigm == 'Randomized'), aes(x=SNR_plot, y=PerCor),
                  geom = "errorbar",fun.data = mean_se, size=.7,width=.2)+
  geom_point(data=dplyr::filter(PC_df, Group == "OHL", Condition != "LISN", Paradigm == 'Randomized'), aes(x=SNR_plot, y=PerCor),
             shape=24,fill = "white",stat="summary", fun=mean, size = 4)+
...
ILE_Group_SNR.main <- lme((ILE) ~ Group+SNR + I(SNR^2) + I(SNR^3), random=~1|Sub,
                 data=dplyr::filter(PC_df, !Run %in% c("Baseline", "WIN_P")), method="ML")

ILE_Group_SNR.int <- lme((ILE) ~ Group*SNR + I(SNR^2)*Group + I(SNR^3)*Group, random=~1|Sub,
                 data=dplyr::filter(PC_df, !Run %in% c("Baseline", "WIN_P")), method="ML")

ILE_Group_SNR_Paradigm.main <- lme((ILE) ~ Group*SNR + I(SNR^2)*Group + I(SNR^3)*Group + Paradigm, random=~1|Sub,
                 data=dplyr::filter(PC_df, !Run %in% c("Baseline", "WIN_P")), method="ML")

ILE_Group_SNR_Paradigm.int <- lme((ILE) ~  Paradigm*SNR*Group + Paradigm*I(SNR^2)*Group + Paradigm*I(SNR^3)*Group, random=~1|Sub,
                 data=dplyr::filter(PC_df, !Run %in% c("Baseline", "WIN_P")), method="ML")

T_Result=anova(ILE_NoCon, ILE_Group, ILE_Group_SNR.main,ILE_Group_SNR.int,ILE_Group_SNR_Paradigm.main,ILE_Group_SNR_Paradigm.int) 

GLMM_Tables(T_Result, Table.Omnibus = T)

```

## ILE: Effect of Group = Yes, ONH - OHL:
```{r, echo=FALSE,results='asis'}
...
# ILE
ILE_NoSNR <- lme((ILE) ~ 1, random=~1|Sub,
                   data=filter(Epoch_List[[1]], Response != "NoRes", Group == Group_i), method="ML")

ILE_SNR <- lme((ILE) ~ SNR + I(SNR^2) + I(SNR^3), random=~1|Sub,
                 data=filter(Epoch_List[[1]], Response != "NoRes", Group == Group_i), method="ML")

ILE_SNR_Paradigm.main <- lme((ILE) ~  Paradigm + SNR + I(SNR^2) + I(SNR^3), random=~1|Sub,
                 data=filter(Epoch_List[[1]], Response != "NoRes", Group == Group_i), method="ML")

ILE_SNR_Paradigm.int <- lme((ILE) ~  Paradigm*SNR + Paradigm*I(SNR^2) + Paradigm*I(SNR^3), random=~1|Sub,
                 data=filter(Epoch_List[[1]], Response != "NoRes", Group == Group_i), method="ML")

T_Result=anova(ILE_NoSNR, ILE_SNR,ILE_SNR_Paradigm.main, ILE_SNR_Paradigm.int)

print(paste("SNR effect on ILE for", Group_i))
 GLMM_Tables(T_Result, Table.model = T)
...
RLE_Group_SNR.main <- lme((RLE) ~ Group+SNR + I(SNR^2) + I(SNR^3), random=~1|Sub,
                 data=dplyr::filter(PC_df, !Run %in% c("Baseline", "WIN_P")), method="ML")

RLE_Group_SNR.int <- lme((RLE) ~ Group*SNR + I(SNR^2)*Group + I(SNR^3)*Group, random=~1|Sub,
                 data=dplyr::filter(PC_df, !Run %in% c("Baseline", "WIN_P")), method="ML")

RLE_Group_SNR_Paradigm.main <- lme((RLE) ~ Group*SNR + I(SNR^2)*Group + I(SNR^3)*Group + Paradigm, random=~1|Sub,
                 data=dplyr::filter(PC_df, !Run %in% c("Baseline", "WIN_P")), method="ML")

RLE_Group_SNR_Paradigm.int <- lme((RLE) ~  Paradigm*SNR*Group + Paradigm*I(SNR^2)*Group + Paradigm*I(SNR^3)*Group, random=~1|Sub,
                 data=dplyr::filter(PC_df, !Run %in% c("Baseline", "WIN_P")), method="ML")

T_Result=anova(RLE_NoCon, RLE_Group, RLE_Group_SNR.main,RLE_Group_SNR.int,RLE_Group_SNR_Paradigm.main,RLE_Group_SNR_Paradigm.int) 

GLMM_Tables(T_Result, Table.Omnibus = T)
```

## RLE: Effect of Group = Yes, ONH - OHL:
```{r, echo=FALSE,results='asis'}
...
for (Group_i in Groups){
# RLE
RLE_NoSNR <- lme((RLE) ~ 1, random=~1|Sub,
                   data=filter(Epoch_List[[1]], Response != "NoRes", Group == Group_i), method="ML")

RLE_SNR <- lme((RLE) ~ SNR + I(SNR^2) + I(SNR^3), random=~1|Sub,
                 data=filter(Epoch_List[[1]], Response != "NoRes", Group == Group_i), method="ML")

RLE_SNR_Paradigm.main <- lme((RLE) ~  Paradigm + SNR + I(SNR^2) + I(SNR^3), random=~1|Sub,
                 data=filter(Epoch_List[[1]], Response != "NoRes", Group == Group_i), method="ML")

RLE_SNR_Paradigm.int <- lme((RLE) ~  Paradigm*SNR + Paradigm*I(SNR^2) + Paradigm*I(SNR^3), random=~1|Sub,
                 data=filter(Epoch_List[[1]], Response != "NoRes", Group == Group_i), method="ML")

T_Result=anova(RLE_NoSNR, RLE_SNR,RLE_SNR_Paradigm.main, RLE_SNR_Paradigm.int)

print(paste("SNR effect on RLE for", Group_i))
 GLMM_Tables(T_Result, Table.model = T)
...
### Required Self-Reported Listening Effort - by group, Descending- Black, Random - White
```{r, echo=FALSE, results='hide'}
Groups = c('ONH', 'OHL')
for (group_i in Groups) {
...
SelfReport_plot=ggplot()+
  ...
  geom_point(data=dplyr::filter(PC_df, Group == group_i,Condition != "LISN", Paradigm == 'Descending'), aes(x=SNR_plot, y=RLE),
             shape=21, fill = "black",stat="summary", fun=mean, size = 4)+
  geom_line (data=dplyr::filter(PC_df, Group == group_i,Condition != "LISN", Paradigm == 'Descending'), aes(x=SNR_plot, y=RLE),
              stat="summary", fun=mean, group=10, linetype='solid', size=1.3)+
  stat_summary(data=dplyr::filter(PC_df, Group == group_i,Condition != "LISN", Paradigm == 'Descending'), aes(x=SNR_plot, y=RLE),
                  geom = "errorbar",fun.data = mean_se, size=.7,width=.2)+
  
  geom_line (data=dplyr::filter(PC_df, Group == group_i,Condition != "LISN", Paradigm == 'Randomized'), aes(x=SNR_plot, y=RLE),
              stat="summary", fun=mean, group=10, linetype='solid',  size=1.3)+
  stat_summary(data=dplyr::filter(PC_df, Group == group_i,Condition != "LISN", Paradigm == 'Randomized'), aes(x=SNR_plot, y=RLE),
                  geom = "errorbar",fun.data = mean_se, size=.7,width=.2)+ 
  geom_point(data=dplyr::filter(PC_df, Group == group_i,Condition != "LISN", Paradigm == 'Randomized'), aes(x=SNR_plot, y=RLE),
             shape=21,fill = "white", stat="summary", fun=mean, size = 4)+
 ...
 # labs(title = "Self-Reported Listening Effort Over WIN SNR")+
   xlab("SIGNAL-TO-NOISE RATIO (dB)")+ ylab("REQUIRED \nSELF-REPORTED EFFORT")+
  labs(title = paste(group_i,'Descending- Black, Random - White'))+
   plot_finish
...
### Invested Self-Reported Listening Effort- ONH, Descending- Black, Random - White
```{r, echo=FALSE, results='hide'}
Groups = c('ONH', 'OHL')
for (group_i in Groups) {
...
SelfReport_plot=ggplot()+
  ...
  geom_point(data=dplyr::filter(PC_df, Group == group_i,Condition != "LISN", Paradigm == 'Descending'), aes(x=SNR_plot, y=ILE),
             shape=21, fill = "black",stat="summary", fun=mean, size = 4)+
  geom_line (data=dplyr::filter(PC_df, Group == group_i,Condition != "LISN", Paradigm == 'Descending'), aes(x=SNR_plot, y=ILE),
              stat="summary", fun=mean, group=10, linetype='solid', size=1.3)+
  stat_summary(data=dplyr::filter(PC_df, Group == group_i,Condition != "LISN", Paradigm == 'Descending'), aes(x=SNR_plot, y=ILE),
                  geom = "errorbar",fun.data = mean_se, size=.7,width=.2)+
  
  geom_line (data=dplyr::filter(PC_df, Group == group_i,Condition != "LISN", Paradigm == 'Randomized'), aes(x=SNR_plot, y=ILE),
              stat="summary", fun=mean, group=10, linetype='solid',  size=1.3)+
  stat_summary(data=dplyr::filter(PC_df, Group == group_i,Condition != "LISN", Paradigm == 'Randomized'), aes(x=SNR_plot, y=ILE),
                  geom = "errorbar",fun.data = mean_se, size=.7,width=.2)+ 
  geom_point(data=dplyr::filter(PC_df, Group == group_i,Condition != "LISN", Paradigm == 'Randomized'), aes(x=SNR_plot, y=ILE),
             shape=21,fill = "white", stat="summary", fun=mean, size = 4)+
...
## Log Transformation of skewed EEG frequency powers
...
## Theta and Alpha with both groups
```{r, echo=FALSE}
Freq_plot = NULL
Freq_plot_IAF = NULL
Freq_plot_Correct = NULL
 for (i in seq_along(Epoch_List)){
   # change alpha theta
       Freq_plot[[i]] = filter(Epoch_List[[i]], Response != "NoRes")%>% 
         dplyr::select( Sub, Theta, Alpha, SNR_plot, Group)%>%pivot_longer(cols = Theta:Alpha, 
                                           names_to = "Frequency",
                                      names_transform = list(Frequency = ~readr::parse_factor(.x, levels = c("Theta", "Alpha"))),
                                           values_to = "Power")
       Freq_plot_IAF[[i]] = filter(Epoch_List[[i]], Response != "NoRes")%>% 
         dplyr::select( Sub, Theta_IAF, Alpha_IAF, SNR_plot, Group)%>%pivot_longer(cols = Theta_IAF:Alpha_IAF, 
                                           names_to = "Frequency",
                                      names_transform = list(Frequency = ~readr::parse_factor(.x, levels = c("Theta_IAF", "Alpha_IAF"))),
                                           values_to = "Power")
      Freq_plot_Correct[[i]] = filter(Epoch_List[[i]], Response == 'Cor')%>% 
         dplyr::select( Sub, Theta, Alpha, SNR_plot, Group)%>%pivot_longer(cols = Theta:Alpha, 
                                           names_to = "Frequency",
                                      names_transform = list(Frequency = ~readr::parse_factor(.x, levels = c("Theta", "Alpha"))),
                                           values_to = "Power")
...
for (ii in Groups) {
 ...
 for (i in seq_along(Freq_plot)){

print(ggplot(data=dplyr::filter(Freq_plot[[i]], !is.na(SNR_plot), Group == ii), 
   ...
   scale_fill_manual(values = Mycolors)+
      ...
      labs(title = paste(ii, Epoch_Win[i],"- Frequency Power Over WIN SNR"))+
       xlab("SIGNAL-TO-NOISE RATIO (dB)")+   
    # ylab(bquote(PSD==10~"*"~log[10](µV^2*"/"~Hz)))+
    coord_cartesian(ylim=c(-1.25, .5))+
 plot_finish)
...
print(ggplot()+
        ...
        geom_line(data=dplyr::filter(Freq_plot[[i]], !is.na(SNR_plot), Group == 'OHL'),
                  aes(size=.8),stat="summary", fun=mean, color = 'black')+
        stat_summary(data=dplyr::filter(Freq_plot[[i]], !is.na(SNR_plot), Group == 'OHL'),
                     aes(size=.7,width=.2),geom = "errorbar", fun.data = mean_se, position = "dodge", color = 'black')+
        geom_point(data=dplyr::filter(Freq_plot[[i]], !is.na(SNR_plot), Group == 'OHL'),
                   aes( size = 3),stat="summary", fun=mean, fill = c('white'), stroke = 1.6)+

        geom_line(data=dplyr::filter(Freq_plot[[i]], !is.na(SNR_plot), Group == 'ONH'),
                  aes(size=.8),stat="summary", fun=mean, color = 'black')+
        stat_summary(data=dplyr::filter(Freq_plot[[i]], !is.na(SNR_plot), Group == 'ONH'),
                     aes(size=.7,width=.2),geom = "errorbar", fun.data = mean_se, position = "dodge", color = 'black')+
        geom_point(data=dplyr::filter(Freq_plot[[i]], !is.na(SNR_plot), Group == 'ONH'),
                   aes( size = 4),stat="summary", fun=mean, color = 'black')+
                   


   scale_shape_manual(values=c(25, 22))+
   scale_color_manual (values = c( 'dodgerblue2', 'red2'))+
    scale_fill_manual(values = c("dodgerblue2","red2", 'white','white'))+
         
  
      labs(title = paste(Epoch_Win[i],"- Frequency Power Over WIN SNR"))+
       xlab("SIGNAL-TO-NOISE RATIO (dB)")+   
     # ylab(bquote(10~"*"~log[10](µV^2*"/"~Hz)))+
...
print(ggplot()+
        ...
        geom_line(data=dplyr::filter(Freq_plot_Correct[[i]], !is.na(SNR_plot), Group == 'OHL'),
                  aes(size=.8),stat="summary", fun=mean, color = 'black')+
        stat_summary(data=dplyr::filter(Freq_plot_Correct[[i]], !is.na(SNR_plot), Group == 'OHL'),
                     aes(size=.7,width=.2),geom = "errorbar", fun.data = mean_se, position = "dodge", color = 'black')+
        geom_point(data=dplyr::filter(Freq_plot_Correct[[i]], !is.na(SNR_plot), Group == 'OHL'),
                   aes( size = 3),stat="summary", fun=mean, fill = c('white'), stroke = 1.6)+

        geom_line(data=dplyr::filter(Freq_plot_Correct[[i]], !is.na(SNR_plot), Group == 'ONH'),
                  aes(size=.8),stat="summary", fun=mean, color = 'black')+
        stat_summary(data=dplyr::filter(Freq_plot_Correct[[i]], !is.na(SNR_plot), Group == 'ONH'),
                     aes(size=.7,width=.2),geom = "errorbar", fun.data = mean_se, position = "dodge", color = 'black')+
        geom_point(data=dplyr::filter(Freq_plot_Correct[[i]], !is.na(SNR_plot), Group == 'ONH'),
                   aes( size = 4),stat="summary", fun=mean, color = 'black')+
...
print(ggplot()+
        aes(x=ILE, y = Power, color = Frequency, fill = Frequency, group = Frequency, shape = Frequency)+
        
        geom_line(data=dplyr::filter(Freq_plot[[i]], !is.na(ILE), Group == 'OHL'),
                  aes(size=.8),stat="summary", fun=mean, color = 'black')+
        stat_summary(data=dplyr::filter(Freq_plot[[i]], !is.na(ILE), Group == 'OHL'),
                     aes(size=.7,width=.2),geom = "errorbar", fun.data = mean_se, position = "dodge", color = 'black')+
        geom_point(data=dplyr::filter(Freq_plot[[i]], !is.na(ILE), Group == 'OHL'),
                   aes( size = 3),stat="summary", fun=mean, fill = c('white'), stroke = 1.6)+

        geom_line(data=dplyr::filter(Freq_plot[[i]], !is.na(ILE), Group == 'ONH'),
                  aes(size=.8),stat="summary", fun=mean, color = 'black')+
        stat_summary(data=dplyr::filter(Freq_plot[[i]], !is.na(ILE), Group == 'ONH'),
                     aes(size=.7,width=.2),geom = "errorbar", fun.data = mean_se, position = "dodge", color = 'black')+
        geom_point(data=dplyr::filter(Freq_plot[[i]], !is.na(ILE), Group == 'ONH'),
                   aes( size = 4),stat="summary", fun=mean, color = 'black')+
...
# Groups = c('ONH', 'OHL')
 ...
 for (i in seq_along(Freq_plot_IAF)){

print(ggplot()+
        aes(x=SNR_plot, y = Power, color = Frequency, fill = Frequency, group = Frequency, shape = Frequency)+
        
        geom_line(data=dplyr::filter(Freq_plot_IAF[[i]], !is.na(SNR_plot), Group == 'OHL'),
                  aes(size=.8),stat="summary", fun=mean, color = 'black')+
        stat_summary(data=dplyr::filter(Freq_plot_IAF[[i]], !is.na(SNR_plot), Group == 'OHL'),
                     aes(size=.7,width=.2),geom = "errorbar", fun.data = mean_se, position = "dodge", color = 'black')+
        geom_point(data=dplyr::filter(Freq_plot_IAF[[i]], !is.na(SNR_plot), Group == 'OHL'),
                   aes( size = 3),stat="summary", fun=mean, fill = c('white'), stroke = 1.6)+

        geom_line(data=dplyr::filter(Freq_plot_IAF[[i]], !is.na(SNR_plot), Group == 'ONH'),
                  aes(size=.8),stat="summary", fun=mean, color = 'black')+
        stat_summary(data=dplyr::filter(Freq_plot_IAF[[i]], !is.na(SNR_plot), Group == 'ONH'),
                     aes(size=.7,width=.2),geom = "errorbar", fun.data = mean_se, position = "dodge", color = 'black')+
        geom_point(data=dplyr::filter(Freq_plot_IAF[[i]], !is.na(SNR_plot), Group == 'ONH'),
                   aes( size = 4),stat="summary", fun=mean, color = 'black')+
   ...
   scale_color_manual (values = c( 'dodgerblue2', 'red2'))+
    scale_fill_manual(values = c("dodgerblue2","red2", 'white','white'))+
      ...
      labs(title = paste(Epoch_Win[i],"- IAF Frequency Power Over WIN SNR"))+
       xlab("SIGNAL-TO-NOISE RATIO (dB)")+   
     # ylab(bquote(10~"*"~log[10](µV^2*"/"~Hz)))+
    coord_cartesian(ylim=c(-1.25, .5))+
 plot_finish)
...
## Theta by group ONH=black eval = false 
...
Theta_Group_SNR.main <- lme((Theta) ~ SNR+Group + I(SNR^2) + I(SNR^3), random=~1|Sub,
                 data=filter(Epoch_List[[i]], Response != "NoRes"), method="ML")

Theta_Group_SNR.int <- lme((Theta) ~ SNR*Group + I(SNR^2)*Group + I(SNR^3)*Group, random=~1|Sub,
                 data=filter(Epoch_List[[i]], Response != "NoRes"), method="ML")

Theta_Group_SNR_Paradigm.main <- lme((Theta) ~  Paradigm + SNR*Group + I(SNR^2)*Group + I(SNR^3)*Group, random=~1|Sub,
                 data=filter(Epoch_List[[i]], Response != "NoRes"), method="ML")

Theta_Group_SNR_Paradigm.int <- lme((Theta) ~  Paradigm*SNR*Group + Paradigm*I(SNR^2)*Group + Paradigm*I(SNR^3)*Group, random=~1|Sub,
                 data=filter(Epoch_List[[i]], Response != "NoRes"), method="ML")

T_Result=anova(Theta_NoSNR, Theta_Group, Theta_Group_SNR.main,Theta_Group_SNR.int,
               Theta_Group_SNR_Paradigm.main,Theta_Group_SNR_Paradigm.int)
print(paste("SNR effect on Theta for", Epoch_Win[i]))
GLMM_Tables(T_Result, Table.model = T)
...
### Examine the null effect of group in carrier phrase
# lm model of data for freq plot
i = 1
lm_freqplot =  lme(Power ~ Group, random=~1|Sub,
                   data=filter(Freq_plot[[i]], Frequency == "Theta"), method="ML")

summary(lm_freqplot)
summary(Theta_Group)


CarrierTheta=filter(Epoch_List[[i]], Response != "NoRes")

summary(CarrierTheta$Theta)
...
Theta_SNR_Paradigm.main <- lme((Theta) ~  Paradigm + SNR + I(SNR^2) + I(SNR^3), random=~1|Sub,
                 data=filter(Epoch_List[[i]], Response != "NoRes", Group == Group_i), method="ML")

Theta_SNR_Paradigm.int <- lme((Theta) ~  Paradigm*SNR + Paradigm*I(SNR^2) + Paradigm*I(SNR^3), random=~1|Sub,
                 data=filter(Epoch_List[[i]], Response != "NoRes", Group == Group_i), method="ML")

T_Result=anova(Theta_NoSNR, Theta_SNR,Theta_SNR_Paradigm.main, Theta_SNR_Paradigm.int)

print(paste("SNR effect on Theta for", Group_i, Epoch_Win[i]))
Sig.Results[[Group_i]][['Theta']][[Epoch_Win[i]]] = GLMM_Tables(T_Result, Table.model = T, Results = T)
...
```{r, echo=FALSE,results='asis'}
## Effects on STD Alpha Power, Effect of Group?
...
Alpha_Group_SNR.main <- lme((Alpha) ~ SNR+Group + I(SNR^2) + I(SNR^3), random=~1|Sub,
                 data=filter(Epoch_List[[i]], Response != "NoRes"), method="ML")

Alpha_Group_SNR.int <- lme((Alpha) ~ SNR*Group + I(SNR^2)*Group + I(SNR^3)*Group, random=~1|Sub,
                 data=filter(Epoch_List[[i]], Response != "NoRes"), method="ML")

Alpha_Group_SNR_Paradigm.main <- lme((Alpha) ~  Paradigm + SNR*Group + I(SNR^2)*Group + I(SNR^3)*Group, random=~1|Sub,
                 data=filter(Epoch_List[[i]], Response != "NoRes"), method="ML")

Alpha_Group_SNR_Paradigm.int <- lme((Alpha) ~  Paradigm*SNR*Group + Paradigm*I(SNR^2)*Group + Paradigm*I(SNR^3)*Group, random=~1|Sub,
                 data=filter(Epoch_List[[i]], Response != "NoRes"), method="ML")

T_Result=anova(Alpha_NoSNR, Alpha_Group, Alpha_Group_SNR.main,Alpha_Group_SNR.int,
               Alpha_Group_SNR_Paradigm.main,Alpha_Group_SNR_Paradigm.int)
print(paste("SNR effect on Alpha for", Epoch_Win[i]))
GLMM_Tables(T_Result, Table.model = T)
...
Alpha_SNR_Paradigm.main <- lme((Alpha) ~  Paradigm + SNR + I(SNR^2) + I(SNR^3), random=~1|Sub,
                 data=filter(Epoch_List[[i]], Response != "NoRes", Group == Group_i), method="ML")

Alpha_SNR_Paradigm.int <- lme((Alpha) ~  Paradigm*SNR + Paradigm*I(SNR^2) + Paradigm*I(SNR^3), random=~1|Sub,
                 data=filter(Epoch_List[[i]], Response != "NoRes", Group == Group_i), method="ML")

T_Result=anova(Alpha_NoSNR, Alpha_SNR,Alpha_SNR_Paradigm.main,Alpha_SNR_Paradigm.int)

print(paste("SNR effect on Alpha for",  Group_i,Epoch_Win[i]))
Sig.Results[[Group_i]][['Alpha']][[Epoch_Win[i]]] = GLMM_Tables(T_Result, Table.model = T, Results = T)
...
Sig.Results
names(Sig.Results)
  kable(Sig.Results[['ONH']][['Theta']][['Late']], format = "html", escape = F, caption = paste0('<b>','ONH Theta Late','<b>'))%>%
  kable_styling( full_width = F)%>%
  column_spec(1,color="black", bold=T)%>%
  row_spec(0:nrow(Sig.Results[['ONH']][['Theta']][['Late']]),color = "black")
...
WIN_SNR.main <- lme(WIN_Score ~ poly(SNR, degree = 3, raw = TRUE), random=~1|Sub,
                 data=filter(Epoch_List[[i]], Response != "NoRes", Group == group_i), method="ML")

WIN_SNR_Paradigm.main <- lme(WIN_Score ~ poly(SNR, degree = 3, raw = TRUE) + Paradigm, random=~1|Sub,
                 data=filter(Epoch_List[[i]], Response != "NoRes", Group == group_i), method="ML")

WIN_SNR_Paradigm.int <- lme(WIN_Score ~ poly(SNR, degree = 3, raw = TRUE) * Paradigm, random=~1|Sub,
                 data=filter(Epoch_List[[i]], Response != "NoRes", Group == group_i), method="ML")

WIN_SNR_Paradigm_Theta.main <- lme(WIN_Score ~ poly(SNR, degree = 3, raw = TRUE) * Paradigm + Theta, random=~1|Sub,
                 data=filter(Epoch_List[[i]], Response != "NoRes", Group == group_i), method="ML")

WIN_SNR_Paradigm_Theta.int <- lme(WIN_Score ~ poly(SNR, degree = 3, raw = TRUE) * Paradigm * Theta, random=~1|Sub,
                 data=filter(Epoch_List[[i]], Response != "NoRes", Group == group_i), method="ML")

WIN_SNR_Paradigm_Theta_Alpha.main <- lme(WIN_Score ~ poly(SNR, degree = 3, raw = TRUE) * Paradigm * Theta + Alpha, random=~1|Sub,
                 data=filter(Epoch_List[[i]], Response != "NoRes", Group == group_i), method="ML")

WIN_SNR_Paradigm_Theta_Alpha.int <- lme(WIN_Score ~ poly(SNR, degree = 3, raw = TRUE) * Paradigm * Theta * Alpha, random=~1|Sub,
                 data=filter(Epoch_List[[i]], Response != "NoRes", Group == group_i), method="ML")



T_Result=anova(WIN_Null, WIN_SNR.main, WIN_SNR_Paradigm.main, WIN_SNR_Paradigm.int,WIN_SNR_Paradigm_Theta.main,
               WIN_SNR_Paradigm_Theta.int,WIN_SNR_Paradigm_Theta_Alpha.main, WIN_SNR_Paradigm_Theta_Alpha.int)

print(paste( group_i,"Effect on WIN for", Epoch_Win[i]))
GLMM_Tables(T_Result, Table.model = T)
}
}
```

### Effect of Alpha & Theta on ILE by Group
...
ILE_SNR.main <- lme(ILE ~ poly(SNR, degree = 3, raw = TRUE), random=~1|Sub,
                 data=filter(Epoch_List[[i]], Response != "NoRes", Group == group_i), method="ML")

ILE_SNR_Paradigm.main <- lme(ILE ~ poly(SNR, degree = 3, raw = TRUE) + Paradigm, random=~1|Sub,
                 data=filter(Epoch_List[[i]], Response != "NoRes", Group == group_i), method="ML")

ILE_SNR_Paradigm.int <- lme(ILE ~ poly(SNR, degree = 3, raw = TRUE) * Paradigm, random=~1|Sub,
                 data=filter(Epoch_List[[i]], Response != "NoRes", Group == group_i), method="ML")

ILE_SNR_Paradigm_Theta.main <- lme(ILE ~ poly(SNR, degree = 3, raw = TRUE) * Paradigm + Theta, random=~1|Sub,
                 data=filter(Epoch_List[[i]], Response != "NoRes", Group == group_i), method="ML")

ILE_SNR_Paradigm_Theta.int <- lme(ILE ~ poly(SNR, degree = 3, raw = TRUE) * Paradigm * Theta, random=~1|Sub,
                 data=filter(Epoch_List[[i]], Response != "NoRes", Group == group_i), method="ML")

ILE_SNR_Paradigm_Theta_Alpha.main <- lme(ILE ~ poly(SNR, degree = 3, raw = TRUE) * Paradigm * Theta + Alpha, random=~1|Sub,
                 data=filter(Epoch_List[[i]], Response != "NoRes", Group == group_i), method="ML")

ILE_SNR_Paradigm_Theta_Alpha.int <- lme(ILE ~ poly(SNR, degree = 3, raw = TRUE) * Paradigm * Theta * Alpha, random=~1|Sub,
                 data=filter(Epoch_List[[i]], Response != "NoRes", Group == group_i), method="ML")



T_Result=anova(ILE_Null, ILE_SNR.main, ILE_SNR_Paradigm.main, ILE_SNR_Paradigm.int,ILE_SNR_Paradigm_Theta.main,
               ILE_SNR_Paradigm_Theta.int,ILE_SNR_Paradigm_Theta_Alpha.main, ILE_SNR_Paradigm_Theta_Alpha.int)

print(paste( group_i,"Effect on ILE for", Epoch_Win[i]))
GLMM_Tables(T_Result, Table.model = T)
}
}
```
...
```{r, echo=FALSE, warning=FALSE}
# jtools approach
# for jtools: using export_summs and the chunk option results = 'asis' will give nice HTML and PTD tables, to.word = TRUE creates Word document with table

# # Create data used for model with polynomials
lm.data=filter(Epoch_List[[3]], Response != "NoRes", Group == 'ONH')
lm.data$SNR2 = lm.data$SNR^2
lm.data$SNR3 = lm.data$SNR^3
# Use lm cause lme output does not work:
ILE_SNR_Paradigm_Theta_Alpha.int <- lm(ILE ~ SNR * Paradigm * Theta * Alpha + SNR2 * Paradigm * Theta * Alpha +
                                         SNR3 * Paradigm * Theta * Alpha, random=~1|Sub, data=lm.data, method="ML")
# pred = first predictor, modx = x axis (line type), mod2 = panels
print("linear SNR")
probe_interaction(ILE_SNR_Paradigm_Theta_Alpha.int, pred = Alpha, modx = Theta, mod2 = SNR, alpha = .05) 
print("SNR ^2")
probe_interaction(ILE_SNR_Paradigm_Theta_Alpha.int, pred = Alpha, modx = Theta, mod2 = SNR2, alpha = .05)
print("SNR ^3")
probe_interaction(ILE_SNR_Paradigm_Theta_Alpha.int, pred = Alpha, modx = Theta, mod2 = SNR3, alpha = .05)

probe_interaction(ILE_SNR_Paradigm_Theta_Alpha.int, pred = SNR, modx = Paradigm, alpha = .05)

probe_interaction(ILE_SNR_Paradigm_Theta_Alpha.int, pred = SNR3, modx = Paradigm, alpha = .05)

probe_interaction(ILE_SNR_Paradigm_Theta_Alpha.int, pred = SNR2, modx = Theta, alpha = .05)
...
### Effect of Alpha & Theta on RLE by Group
...
RLE_SNR.main <- lme(RLE ~ poly(SNR, degree = 3, raw = TRUE), random=~1|Sub,
                 data=filter(Epoch_List[[i]], Response != "NoRes", Group == group_i), method="ML")

RLE_SNR_Paradigm.main <- lme(RLE ~ poly(SNR, degree = 3, raw = TRUE) + Paradigm, random=~1|Sub,
                 data=filter(Epoch_List[[i]], Response != "NoRes", Group == group_i), method="ML")

RLE_SNR_Paradigm.int <- lme(RLE ~ poly(SNR, degree = 3, raw = TRUE) * Paradigm, random=~1|Sub,
                 data=filter(Epoch_List[[i]], Response != "NoRes", Group == group_i), method="ML")

RLE_SNR_Paradigm_Theta.main <- lme(RLE ~ poly(SNR, degree = 3, raw = TRUE) * Paradigm + Theta, random=~1|Sub,
                 data=filter(Epoch_List[[i]], Response != "NoRes", Group == group_i), method="ML")

RLE_SNR_Paradigm_Theta.int <- lme(RLE ~ poly(SNR, degree = 3, raw = TRUE) * Paradigm * Theta, random=~1|Sub,
                 data=filter(Epoch_List[[i]], Response != "NoRes", Group == group_i), method="ML")

RLE_SNR_Paradigm_Theta_Alpha.main <- lme(RLE ~ poly(SNR, degree = 3, raw = TRUE) * Paradigm * Theta + Alpha, random=~1|Sub,
                 data=filter(Epoch_List[[i]], Response != "NoRes", Group == group_i), method="ML")

RLE_SNR_Paradigm_Theta_Alpha.int <- lme(RLE ~ poly(SNR, degree = 3, raw = TRUE) * Paradigm * Theta * Alpha, random=~1|Sub,
                 data=filter(Epoch_List[[i]], Response != "NoRes", Group == group_i), method="ML")



T_Result=anova(RLE_Null, RLE_SNR.main, RLE_SNR_Paradigm.main, RLE_SNR_Paradigm.int,RLE_SNR_Paradigm_Theta.main,
               RLE_SNR_Paradigm_Theta.int,RLE_SNR_Paradigm_Theta_Alpha.main, RLE_SNR_Paradigm_Theta_Alpha.int)

print(paste( group_i,"Effect on RLE for", Epoch_Win[i]))
GLMM_Tables(T_Result, Table.model = T)
}
}
```
...
a <- ggplot(dplyr::filter(EEG_LE, !is.na(SNR_plot), Group == 'ONH'), aes(x=SNR_plot, y=(Theta)))+
  ...
  facet_wrap(facets = vars(Sub))+
    labs(title = "ONH - Theta Frequency Power of WIN SNR")+
   xlab("SIGNAL-TO-NOISE RATIO (dB)")+ ylab("POWER SPECTRUM \nDENSITY (PSD)")

a

a <- ggplot(dplyr::filter(EEG_LE, !is.na(SNR_plot), Group == 'OHL'), aes(x=SNR_plot, y=(Theta)))+
    geom_point(color = "blue2", stat="summary", fun=mean)+
    geom_line(stat="summary", fun=mean, group=10, size=1.2, color="blue2")+
    stat_summary(  geom = "errorbar",fun.data = mean_se, color="blue3",size=.7, width=.25)+
  facet_wrap(facets = vars(Sub))+
    labs(title = "OHL - Theta Frequency Power of WIN SNR")+
   xlab("SIGNAL-TO-NOISE RATIO (dB)")+ ylab("POWER SPECTRUM \nDENSITY (PSD)")
