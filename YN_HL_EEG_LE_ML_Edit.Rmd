...
10 is the optimal number of variables at each node, this is all of the variables. Lets look at what is the best setting for the test data: 
```{r}
mtry.list = 2:10
Mean.Error = rep(0, 9)
...
for (i in 1:length(mtry.list)) {
  ...
  for (ii in 1:50) {
     EEG.model = randomForest(formula = Mean.LE.SNR ~ ., 
                         data = EEG.db[['train']][colnames(EEG.db[['train']]) %in% wanted], ntree = ntree.best, mtry = mtry.list[i], importance=TRUE)
pred = predict(EEG.model, EEG.db[['test']][colnames(EEG.db[['test']]) %in% wanted])
...
Try Random Forest again with additional variables (alias var removed)
