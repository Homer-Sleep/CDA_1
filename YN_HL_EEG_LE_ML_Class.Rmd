...
PTA_Demo_Raw = PTA_Demo_Raw[PTA_Demo_Raw$Sub != 'YN_22', ] # remove YN_22
...
# by Group
 ...
 geom_histogram(aes(y = ..count../7), bins = 25, color="#e9ecef", alpha=0.5, position = 'identity')+
   geom_vline(xintercept = 18.4, linetype="dashed", color = "grey51", size=1.3)+
   
   scale_y_continuous (limits = c(0,8),breaks = (0:8))+
   scale_x_reverse(limits = c(25,7),breaks = (25:7))+
   theme_classic()+
   ylab('Participant Count')+
   xlab('Peak Listening Effort SNR'))
...
Given that clinicians want cut points and that those that have normal or minimal listening effort are not the target group, this approach will classify two groups of Above Normal and Normal with a cut point of 18.5 SNR. 

```{r}
# Make classification variable
EEG.ml$label = as.numeric(ifelse(EEG.ml$Mean.LE.SNR <= 18.5,  '0',
                     ifelse(EEG.ml$Mean.LE.SNR < 24, '1','1')))

# Fix Col name issue
setnames(EEG.ml, "Veteran?","Veteran", skip_absent=TRUE)

# remove unwanted variables:
additional.HE = c('TFI', 'Left SRT', 'Right SRT', 'Left Word Rec', 'Right Word Rec', 'SII Left', 'SII Right' )
toRemove = c('SNR', 'SAE', 'PCE', "SNR_num","SNR_inver",'SNR_plot',"Neuroeconomic", "Proxy1", "WIN Test Level", 'MoCA Extra Point?', "Adj_MoCA", 'Handedness', 'Veteran', 'Gender',  "Theta","Late.Theta", "Alpha", "Mean.LE.SNR", 'Ethnicity', 'Race')# Have to remove Ethnicity, only one non-white
toRemove = c(additional.HE, toRemove) 
...
### Test Train split
The test group was from a random sample with 2 from the ONH and OHL groups. 
...
test.obs = list()
for (i in Groups) {
  thisdt = dplyr::filter(EEG.ml.wider, Group == i)
  sub.names = unique(thisdt$Sub)
  if (i == 'ONH'){
    test.obs[[i]] = sample(sub.names, size = length(sub.names)*.3 )
  }else{
  test.obs[[i]] = sample(sub.names, size = length(sub.names)*.2 )
  }
}
test.obs = c(test.obs[['YN']], test.obs[['ONH']], test.obs[['OHL']])

paste('Test Sample Includes:')
test.obs
```

## Distribution of Peak LE SNR by train/test data sets.
```{r}
train_LE_SNR = dplyr::select(EEG.ml, Sub, Group, Mean.LE.SNR)%>% dplyr:: filter(!Sub %in% test.obs)
test_LE_SNR = dplyr::select(EEG.ml, Sub, Group, Mean.LE.SNR)%>% dplyr:: filter(Sub %in% test.obs)
train_test = c('train_LE_SNR', 'test_LE_SNR')
for (i in train_test){
  this_df = get(i)
 print(this_df%>%ggplot(aes(x = Mean.LE.SNR, fill = Group ))+
 geom_histogram(aes(y = ..count../7), bins = 25, color="#e9ecef", alpha=0.5, position = 'identity')+
   geom_vline(xintercept = 18.4, linetype="dashed", color = "grey51", size=1.3)+
  ...
  print(quantile(this_df$Mean.LE.SNR))
  }

```


## Create three variable data sets to predict listening effort catigory 1) EEG, WIN, and Clinical 2) WIN and Clinical 3) Clnical
Update, just run Clinical data set
```{r}

# # Create EEG, WIN, and Clinical data sets: 
# EEG.train =  dplyr::filter(EEG.ml.wider, !Sub %in% test.obs)# remove Subs in test.obs
# EEG.test =  dplyr::filter(EEG.ml.wider, Sub %in% test.obs)# keep Subs in test.obs
# 
# # Create WIN and Clinical data sets:
# 
# WIN.Clinic.train = dplyr::select(EEG.train, -c(Alpha.d1,LTheta.d1,Alpha.peak,LTheta.peak))
# WIN.Clinic.test = dplyr::select(EEG.test, -c(Alpha.d1,LTheta.d1,Alpha.peak,LTheta.peak))

# Create Clinical data sets:
Clinic.train = dplyr::select(EEG.train, c(Sub, Age,Education,L250,L500,L1000,L2000,L3000, L4000,L6000,
                 L8000,R250,R500,R1000,R2000,R3000,R4000, R6000,R8000,PTA,HF_PTA, label))
Clinic.test = dplyr::select(EEG.test, c(Sub, Age,Education,L250,L500,L1000,L2000,L3000, L4000,L6000,
                 L8000,R250,R500,R1000,R2000,R3000,R4000, R6000,R8000,PTA,HF_PTA, label))
...
```{r}
#create table for model result comparison: logistic Regression (LR), Random Forest (RF), XGBoost (XGB)
Model_Test_Result = data.frame(Model = c('Logistic',  'Lasso', 'ElasticNet',
                                     'ExpandedElasticNet','RandomForest', 'XGBoost'), 
                           Accuracy = rep(NA, 6), Acc_p = rep(NA, 6), Sensitivity = rep(NA, 6), Specificity = rep(NA, 6), 
                           Bal_Acc = rep(NA, 6))
...
```{r}
  ...
  #logistic model
  train_data = Clinic.train[,-1]
  test_data = Clinic.test[,-1]
  
  this_model = glm(label ~ ., data=train_data, family = 'binomial'(link = 'logit'))
 
  # predict
  pred = predict(this_model, test_data, type = 'response')
  prediction = as.numeric(pred > 0.5)

  # confusion matrix of test set
  this_result = confusionMatrix(factor(prediction),
                factor(test_data$label),
                mode = "everything")
  # select measures of classification
  Model_Test_Result[1,2] = this_result[["overall"]][["Accuracy"]]
  Model_Test_Result[1,3] = this_result[["overall"]][["AccuracyPValue"]]
  Model_Test_Result[1,4] = this_result[["byClass"]][["Sensitivity"]]
  Model_Test_Result[1,5] = this_result[["byClass"]][["Specificity"]]
  Model_Test_Result[1,6] = this_result[["byClass"]][["Balanced Accuracy"]]
  Model_Test_Result
...
train_output = Clinic.train$label 
# make parameters matrix
train_data = data.matrix(Clinic.train[!colnames(Clinic.train) == 'label'])

# Test data set:
# make output variable
test_output = Clinic.test$label 
# make parameters matrix
test_data = data.matrix(Clinic.test[!colnames(Clinic.test) == 'label'])

lambda_seq = 10^seq(2, -2, by = -.1)
EEG.lasso = cv.glmnet(train_data, train_output , alpha = 1, nfolds = 6, lambda = lambda_seq ) # run 6-fold cross-validation of lasso

# plot test MSE by lambda value
plot( EEG.lasso )
  
# find coefficients of best model:
best_model = glmnet(train_data, train_output , alpha = 1, lambda = EEG.lasso$lambda.min)
...
# predict
pred <- predict( best_model, s = EEG.lasso$lambda.min, newx = test_data, type = 'response')
prediction = as.numeric(pred > 0.5)
...
this_result = confusionMatrix(factor(prediction),
                ...
                mode = "everything")
# select measures of classification
Model_Test_Result[2,2] = this_result[["overall"]][["Accuracy"]]
Model_Test_Result[2,3] = this_result[["overall"]][["AccuracyPValue"]]
Model_Test_Result[2,4] = this_result[["byClass"]][["Sensitivity"]]
Model_Test_Result[2,5] = this_result[["byClass"]][["Specificity"]]
Model_Test_Result[2,6] = this_result[["byClass"]][["Balanced Accuracy"]]
Model_Test_Result
  
```

### Elastic-Net, avg of 50 interations #3

```{r}
#Data prep
train_data = Clinic.train
train_data$label = factor(train_data$label)

# # make output variable
# train_output = Clinic.train$label 
# # make parameters matrix
train_data_NoLabel = data.matrix(Clinic.train[!colnames(Clinic.train) == 'label'])
...
for (i in 1:50){
el.net = train(label ~., data =train_data, method = "glmnet", trControl = cv_6)
...
print('how often was alpha == 1?')
 ...
 # New model with best results
  # find coefficients of best model:
  best_model = glmnet(train_data_NoLabel, train_output , alpha = .55, lambda = min(el.net.result$lambda))
  EEG.best.model.coeff=  coef(best_model)
...
# predict
pred <- predict( best_model, newx = test_data, type = 'response')
prediction = as.numeric(pred > 0.5)
...
this_result = confusionMatrix(factor(prediction),
                ...
                mode = "everything")
# select measures of classification
Model_Test_Result[3,2] = this_result[["overall"]][["Accuracy"]]
Model_Test_Result[3,3] = this_result[["overall"]][["AccuracyPValue"]]
Model_Test_Result[3,4] = this_result[["byClass"]][["Sensitivity"]]
Model_Test_Result[3,5] = this_result[["byClass"]][["Specificity"]]
Model_Test_Result[3,6] = this_result[["byClass"]][["Balanced Accuracy"]]
Model_Test_Result
```
...
el.net.result = data.table(alpha = as.numeric(rep(NA, 50)), lambda = as.numeric(rep(NA, 50)), 
                           RMSE = as.numeric(rep(NA, 50)), Rsquared = as.numeric(rep(NA, 50)))
for (i in 1:50){
el.net = train(label ~.^2, data = train_data, method = "glmnet", trControl = cv_6, tuneLength = 10)
el.net.result[i,]= get_best_result(el.net)[1,1:4]
}
...
# New model with best results
# find coefficients of best model:
best_model = glmnet(train_data_NoLabel, train_output , alpha = best.el.net$alpha, lambda = best.el.net$lambda)
EEG.best.model.coeff=  coef(best_model)
print( EEG.best.model.coeff)
...
this_result = confusionMatrix(factor(prediction),
                ...
                mode = "everything")
# select measures of classification
Model_Test_Result[4,2] = this_result[["overall"]][["Accuracy"]]
Model_Test_Result[4,3] = this_result[["overall"]][["AccuracyPValue"]]
Model_Test_Result[4,4] = this_result[["byClass"]][["Sensitivity"]]
Model_Test_Result[4,5] = this_result[["byClass"]][["Specificity"]]
Model_Test_Result[4,6] = this_result[["byClass"]][["Balanced Accuracy"]]
Model_Test_Result
...
prediction = predict(RF_model, test_data, type = 'response')
 ...
 # prediction = as.numeric(pred > 0.5)
  ...
  # confusion matrix of test set
  this_result = confusionMatrix(prediction,
                test_data$label,
                mode = "everything")
  # select measures of classification
  Model_Test_Result[5,2] = this_result[["overall"]][["Accuracy"]]
  Model_Test_Result[5,3] = this_result[["overall"]][["AccuracyPValue"]]
  Model_Test_Result[5,4] = this_result[["byClass"]][["Sensitivity"]]
  Model_Test_Result[5,5] = this_result[["byClass"]][["Specificity"]]
  Model_Test_Result[5,6] = this_result[["byClass"]][["Balanced Accuracy"]]
  Model_Test_Result
...
```{r}
# make output variable
train_output = factor(Clinic.train$label)
# # make parameters matrix
train_data_NoLabel = Clinic.train[!colnames(Clinic.train) == 'label']

# Test data set:
# make output variable
test_output = factor(Clinic.test$label)
# make parameters matrix
test_data = Clinic.test[!colnames(Clinic.test) == 'label']


#From: https://www.listendata.com/2014/11/random-forest-with-r.html

mtry <- tuneRF(train_data_NoLabel, train_output, ntreeTry=3500,
...
train_data = as.data.table(Clinic.train)
test_data = as.data.table(Clinic.test)

# grouping variables need to be 'one hot' 
# # find factor variables
# colnames(Filter(is.factor, train_data))
# Group.Matrix.train = model.matrix(~Group-1, train_data)
# Group.Matrix.test = model.matrix(~Group-1, test_test)
...
# Make label matrix
train.label = as.matrix(train_data$label)
test.label = as.matrix(test_data$label)
...
# train_data [, Mean.LE.SNR :=NULL]
# EEG.db.test[, Mean.LE.SNR :=NULL]

# change into Dmatrix, remove all non-numeric columns (Sub)
EEG.dm.train = xgb.DMatrix(data = as.matrix(train_data[,-1]), label = train.label)# -1 to remove Sub
EEG.dm.test = xgb.DMatrix(data = as.matrix(test_data[,-1]), label = test.label)
```
...
this_result = confusionMatrix(factor(OOF_prediction$max_prob),
                factor(OOF_prediction$label),
                mode = "everything")

# select measures of classification
Model_Train_Result[6,2] = this_result[["overall"]][["Accuracy"]]
Model_Train_Result[6,3] = this_result[["overall"]][["AccuracyPValue"]]
Model_Train_Result[6,4] = this_result[["byClass"]][["Sensitivity"]]
Model_Train_Result[6,5] = this_result[["byClass"]][["Specificity"]]
Model_Train_Result[6,6] = this_result[["byClass"]][["Balanced Accuracy"]]
Model_Train_Result
...
# confusion matrix of test set
this_result = confusionMatrix(factor(test_prediction$max_prob),
                factor(test_prediction$label),
                mode = "everything")
# select measures of classification
Model_Test_Result[6,2] = this_result[["overall"]][["Accuracy"]]
Model_Test_Result[6,3] = this_result[["overall"]][["AccuracyPValue"]]
Model_Test_Result[6,4] = this_result[["byClass"]][["Sensitivity"]]
Model_Test_Result[6,5] = this_result[["byClass"]][["Specificity"]]
Model_Test_Result[6,6] = this_result[["byClass"]][["Balanced Accuracy"]]
Model_Test_Result
...
kable(Model_Test_Result, row.names= F)%>%kable_styling(bootstrap_options = c(  "condensed"),full_width = F)%>%
  column_spec(1,color="black", bold=T)%>%
  row_spec(0:nrow(Model_Test_Result),color = "black")
