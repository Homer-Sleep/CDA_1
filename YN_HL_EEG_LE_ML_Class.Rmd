...
Model_Test_Result = data.frame(Model = c('Logistic',  'Lasso', 
                                     'ExpandedElasticNet','RandomForest', 'XGBoost'), 
                           Accuracy = rep(NA, 5),  Sensitivity = rep(NA, 5), Specificity = rep(NA, 5), 
                           Bal_Acc = rep(NA, 5))

Model_Train_Result = Model_Test_Result

add_train = function(this_result = this_result, Model_i){
  Model_Train_Result[Model_i,2] = round(this_result[["overall"]][["Accuracy"]], digits = 3)
  Model_Train_Result[Model_i,3] = round(this_result[["byClass"]][["Sensitivity"]], digits = 3)
  Model_Train_Result[Model_i,4] = round(this_result[["byClass"]][["Specificity"]], digits = 3)
  Model_Train_Result[Model_i,5] = round(this_result[["byClass"]][["Balanced Accuracy"]], digits = 3)
  return(Model_Train_Result)
}

add_test = function(this_result = this_result, Model_i){
  Model_Test_Result[Model_i,2] = round(this_result[["overall"]][["Accuracy"]], digits = 3)
  Model_Test_Result[Model_i,3] = round(this_result[["byClass"]][["Sensitivity"]], digits = 3)
  Model_Test_Result[Model_i,4] = round(this_result[["byClass"]][["Specificity"]], digits = 3)
  Model_Test_Result[Model_i,5] = round(this_result[["byClass"]][["Balanced Accuracy"]], digits = 3)
...
test_data = Clinic.test[,-1]
 # add cv:
set.seed(42)
cv_num = trainControl(method = "cv", number = 10)
logit.result = data.table(alpha = as.numeric(rep(NA, 50)), lambda = as.numeric(rep(NA, 50)), 
                           RMSE = as.numeric(rep(NA, 50)), Rsquared = as.numeric(rep(NA, 50)))
# for (i in 1:10){
logit.ml = train(label ~.^2, 
               data = train_data, 
               method = "glm", 
               family = 'binomial',
               trControl = cv_num, 
...
# exp(coef(logit.ml$finalModel))

# summary(logit.ml)
# predict Test results
pred = predict(logit.ml, train_data)
prediction = as.numeric(pred > 0.5)

# training results:
this_result = confusionMatrix(factor(prediction),
              factor(train_data$label),
              mode = "everything")
# select measures of classification
Model_Train_Result = add_train(this_result = this_result, Model_i = 1)

# Test results:
# predict
pred = predict(logit.ml, test_data)
prediction = as.numeric(pred > 0.5)
...
this_result = confusionMatrix(factor(prediction),
              ...
              mode = "everything")
# select measures of classification
Model_Test_Result = add_test(this_result = this_result, Model_i = 1)
...
# run 6-fold cross-validation of lasso
EEG.lasso = cv.glmnet(train_data, train_output , alpha = 1, nfolds = 10, lambda = lambda_seq ) 
...
pred = predict( best_model, s = EEG.lasso$lambda.min, newx = train_data, type = 'response')
prediction = as.numeric(pred > 0.5)
...
this_result = confusionMatrix(factor(prediction),
              ...
              mode = "everything")
# select measures of classification
Model_Train_Result = add_train(this_result = this_result, Model_i = 2)
...
this_result = confusionMatrix(factor(prediction),
                ...
                mode = "everything")
# select measures of classification
  Model_Test_Result = add_test(this_result = this_result, Model_i = 2)
...
# 
# # for (i in 1:50){
# el.net = train(label ~., data =train_data, method = "glmnet", trControl = cv_num)
# # el.net.result[i,]= get_best_result(el.net)[1,1:4]
# # }
# el.net.result= get_best_result(el.net)
# 
...
#   # find coefficients of best model:
#   best_model = glmnet(train_data_NoLabel, train_output , alpha = el.net.result$alpha,
#                       lambda = min(el.net.result$lambda))
#   EEG.best.model.coeff=  coef(best_model)
...
#                 mode = "everything")
# # select measures of classification
#   Model_Test_Result = add_test(this_result = this_result, Model_i = 3)
```

## Elastic Net Expanded # 3
...
best.el.net = get_best_result(el.net)
...
# predict Train results
pred <- predict( best_model, newx = train_data_NoLabel, type = 'response')
prediction = as.numeric(pred > 0.5)
...
this_result = confusionMatrix(factor(prediction),
              ...
              mode = "everything")
# select measures of classification
Model_Train_Result = add_train(this_result = this_result, Model_i = 3)
...
# predict Train data
prediction = predict(RF_model, train_data, type = 'response')
 ...
 # prediction = as.numeric(pred > 0.5)
  ...
  this_result = confusionMatrix(prediction,
                ...
                mode = "everything")
  # select measures of classification
  Model_Train_Result = add_train(this_result = this_result, Model_i = 4)
...
prediction = predict(RF_model, test_data, type = 'response')
 ...
 # prediction = as.numeric(pred > 0.5)
  ...
  this_result = confusionMatrix(prediction,
                ...
                mode = "everything")
  # select measures of classification
  Model_Test_Result = add_test(this_result = this_result, Model_i = 4)
...
# select measures of classification
Model_Train_Result = add_train(this_result = this_result, Model_i = 5)
...
this_result = confusionMatrix(factor(test_prediction$max_prob),
                ...
                mode = "everything")
# select measures of classification
  Model_Test_Result = add_test(this_result = this_result, Model_i = 5)
...
```{r}
kable(Model_Train_Result, row.names= F, caption = '<b>Training Data Results</b>')%>%
  kable_styling(bootstrap_options = c(  "condensed"),full_width = F)%>%
  column_spec(1,color="black", bold=T)%>%
  row_spec(0:nrow(Model_Train_Result),color = "black")

kable(Model_Test_Result, row.names= F, caption = '<b>Test Data Results</b>')%>%
  kable_styling(bootstrap_options = c(  "condensed"),full_width = F)%>%
  column_spec(1,color="black", bold=T)%>%
