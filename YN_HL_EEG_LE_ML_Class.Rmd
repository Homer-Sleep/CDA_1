...
toRemove = c('SNR', 'SAE', 'PCE', "SNR_num","SNR_inver",'SNR_plot',"Neuroeconomic", "Proxy1", "WIN Test Level", 'MoCA Extra Point?', "Adj_MoCA", 'Handedness', 'Veteran', 'Gender',  "Theta","Late.Theta", "Alpha", "Mean.LE.SNR")
...
```{r}
#create table for model result comparison: logistical Regression (LR), Random Forest (RF), XGBoost (XGB)
Model.Results = data.frame(Model = c('LR.All', 'RF.All', 'XGB.All', 'LR.Lasso', 'RF.Lasso', 'XGB.Lasso', 'LR.EL', 'RF.EL', 'XGB.EL',
                                     'LR.Inter.EL', 'RF.Inter.EL','XGB.Inter.EL'), 
                           R2 = rep(NA, 12), RMSE = rep(NA, 12), Acc = rep(NA, 12))
```
...
TrainSetNames = c('EEG.train', 'WIN.Clinic.train', 'Clinic.train')
TestSetNames = c('EEG.test', 'WIN.Clinic.test', 'Clinic.test')

for (i in 1:length(TrainSetNames)){
  print(paste('Results for', TrainSetNames[i]))
  #logistic model
  train_data = get(TrainSetNames[i])[,-1]
  test_data = get(TestSetNames[i])[,-1]
  this_model = glm(label ~ ., data=train_data, family = 'binomial'(link = 'logit'))
 
  # predict
  pred = predict(this_model, get(TestSetNames[i])[,-1], type = 'response')
  prediction = as.numeric(pred > 0.5)

  # confusion matrix of test set
  print(confusionMatrix(factor(prediction),
...
## Logistic regression and Variance Inflation Factor (VIF)
...
for (i in 1:length(TrainSetNames)){
  ...
  tried = try(car::vif(this_model))
  
  if (inherits(tried, "try-error")){
    # collect alias variables and add 'Sub' to be removed from model
    alias_list = rownames(alias(this_model)[["Complete"]])
    print(paste(TrainSetNames[i], 'Had alias. Will rerun model with these variables removed'))
    print(alias_list)
    # run model without alias:
    this_model = glm(label ~ ., data=train_data[!colnames(train_data) %in% alias_list], family = 'binomial'(link = 'logit')) 
  } # if alias error end
  ...
  # confusion matrix of test set
Result = confusionMatrix(factor(prediction),
                factor(test_data$label),
                mode = "everything")
}
...
Logistic regression with the clinical variables has comparable accuracy as models with more variables. 



### Try Random Forest and linear regression with all variables
