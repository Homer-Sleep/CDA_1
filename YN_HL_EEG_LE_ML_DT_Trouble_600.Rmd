...
```{r, eval=FALSE}
EEG.ml = as.data.table(EEG_WIN_PTA_ML[[2]]) # just looking at Word epoch 
setkey(EEG.ml, Sub)
EEG.ml$Late.Theta = EEG_WIN_PTA_ML[[3]]$Theta # add late theta
EEG.ml$Group = factor(EEG.ml$Group, levels = c('YN', 'ONH', 'OHL'))
# Many char columns should be numeric:
# Get all character columns
CharacterCols = EEG.ml%>% select_if(is.character)%>%colnames()
CharacterCols = CharacterCols[c(2,7,10:137)] # limit to everything after Demographics
EEG.ml[, (CharacterCols) := lapply(.SD, as.numeric), .SDcols = CharacterCols] # change the selected columns to numeric
...
# EEG.ml$Ceiling.SNR = as.numeric(NA)
# EEG.ml$Alpha.d1 = NA
# EEG.ml$LTheta.d1 = NA
...
for (Sub.i in seq_along(Subjects)) {
  ...
  EEG.ml[Subjects[Sub.i],Ceiling.SNR := (Ceiling.SNR)]  # save to df
  ...
  ILE.d1 = which.max(abs(diff(EEG.ml[with(EEG.ml, Sub == Subjects[Sub.i] & SNR_num %in% id),]$ILE)))
                                                            # diff(x)=1 so no need to divide 
  EEG.ml[Subjects[Sub.i], ILE.d1 := (SNRs.d1[ILE.d1])]
     
  RLE.d1 = which.max(abs(diff(EEG.ml[with(EEG.ml, Sub == Subjects[Sub.i] & SNR_plot %in% id),]$RLE)))
  EEG.ml[Subjects[Sub.i], RLE.d1 := (SNRs.d1[RLE.d1])] 
 
  # Mean SNR of all above predictors:
  EEG.ml[Subjects[Sub.i], Mean.LE.SNR := (with(EEG.ml[with(EEG.ml, Sub == Subjects[Sub.i]),], 
